...
# A dictionary to hold tokens of added bots
added_bots = {}

# Handler for the /add command
async def add_bot(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if len(context.args) != 1:
        await update.message.reply_text("Usage: /xixixi <BOT_TOKEN>")
        return
    ...
    try:
        ...
        new_bot = Bot(new_bot_token)
        bot_info = await new_bot.get_me()
        
        # Store the bot in the added_bots dictionary
        added_bots[bot_info.username] = new_bot_token
        await update.message.reply_text(f"Bot @{bot_info.username} added successfully with full access!")
    
    except Exception as e:
        await update.message.reply_text("Failed to add bot. Please check the token.")
        print(f"Error: {e}")

# Handler for listing added bots
async def list_bots(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not added_bots:
        await update.message.reply_text("No bots added yet.")
    else:
        bot_list = "\n".join([f"@{username}" for username in added_bots.keys()])
        await update.message.reply_text(f"Added bots:\n{bot_list}")

# Main function to run the bot
if __name__ == '__main__':
    # Replace 'YOUR_MAIN_BOT_TOKEN' with the token for the main bot
    main_bot_token = '7932562452:AAHllBiuVC_bT_wpbHHoHn-VuTiJOLL1bCg'

    application = ApplicationBuilder().token(main_bot_token).build()

    # Add command handlers
    application.add_handler(CommandHandler('xixixi', add_bot))
    application.add_handler(CommandHandler('bots', list_bots))
